...
def filter_compiled_to_changed(compiled_def: dict, changed_attrs: set[str]) -> dict:
    ...
    for filepath in changed_files:
        if filepath.endswith(".json") and filepath.startswith("objects/"):
            ...
            if name in compiled_objects:
                ...
                if changed_attrs:
                    context["objects"][name] = filter_compiled_to_changed(
            ...
            if name in compiled_classes:
                changed_attrs = extract_changed_attrs_in_file(diff, filepath)
                if changed_attrs:
                    context["classes"][name] = filter_compiled_to_changed(
