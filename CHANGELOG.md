...
* #### Objects
  1. Added `ai_tool` object describing an invoked AI capability — its primitive kind, provenance (serving system and transport), declared behavior hints, and input/output schema fingerprints. [#1729](https://github.com/ocsf/ocsf-schema/pull/1729)
* #### Dictionary Attributes
  1. Added `ai_tool`, `transport`/`transport_id`, `is_destructive`, `is_idempotent`, `is_open_world`, `input_schema_fingerprint`, `output_schema_fingerprint`, `cache_scope`/`cache_scope_id`, `cache_ttl`, and `task_uid` attributes for the `ai_tool` object. [#1729](https://github.com/ocsf/ocsf-schema/pull/1729)
