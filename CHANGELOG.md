...
### Added
* #### Categories
  1. Added `Physical Security` category (uid 9) for activity in the physical world: access control decisions, alarm signals and their disposition, guard patrols, post staffing, response dispatch, and incident reports.
* #### Event Classes
  1. Added `Physical Access Activity` (9001) event class to the Physical Security category: access decisions and portal state changes, following the ONVIF Access Control and Door Control service vocabularies.
  1. Added `Alarm Signal Activity` (9002) event class to the Physical Security category: alarm signals from monitored points, crosswalked to SIA DC-03 and Ademco Contact ID.
  1. Added `Alarm Disposition Activity` (9003) event class to the Physical Security category: the validated outcome of a signal, with ANSI/TMA AVS-01 validation levels.
  1. Added `Patrol Activity` (9004) event class to the Physical Security category: guard tours, checkpoint registrations, and their exceptions.
  1. Added `Post Staffing Activity` (9005) event class to the Physical Security category: post assumption, relief, no-shows, and coverage gaps.
  1. Added `Response Activity` (9006) event class to the Physical Security category: dispatch and progress of a response resource through assigned, en route, on scene, and cleared.
  1. Added `Incident Report Activity` (9007) event class to the Physical Security category: incident reports carrying a normalized incident type beside the reporting organization's own.
* #### Profiles
...
* #### Objects
  ...
  1. Added `portal`, `physical_credential`, `alarm_point`, `signal_code`, `post`, `checkpoint`, `patrol_tour`, `response_resource`, and `disposition_details` objects for the Physical Security classes.
* #### Observables
* #### Platform Extensions
* #### Dictionary Attributes
